    ...
    <footer className="site-footer" data-testid="site-footer">
      <Container>
        ...
        <div className="site-footer__grid">
          ...
          <div><span>Connect</span><a href={contactContent.phoneHref}>{contactContent.phoneDisplay}</a>{socialLinks.map((item) => item.href ? <a key={item.label} href={item.href}>{item.label}</a> : <span className="site-footer__pending" key={item.label}>{item.label} · pending</span>)}</div>
          <div><span>Coverage</span><p>{siteNotice}</p></div>
        </div>
        <div className="site-footer__bottom">
          <p>© {new Date().getFullYear()} Liberty Security. Company and licence details pending approval.</p>
          <PrivacyButton />
